...
class RecetteDAO {
    ...
    public function trouverRecettesCategorie($categorieNom) {
        if($categorieNom == "" || is_string($categorieNom)){
            throw new InvalidArgumentException("categorie Nom invalide");
        }
        try {
    ...
    public function ajouterRecetteIngredient($idRecette, $idIngredient, $quantite) {
        if($idRecette == "" || is_string($idRecette) || $idIngredient == "" || is_string($idIngredient) || $quantite ==""){
            throw new InvalidArgumentException("idRecette, idIngredient, quantite Invalide");
        }
        try {
    ...
    public function getQuantitesIngredientsRecette($nomRecette) {
        try {
            $requete = $this->bdd->prepare("
                SELECT quantite FROM recette_ingredient WHERE id_recette = (SELECT id FROM recettes WHERE nom = ?)
            ");
            $requete->execute([$nomRecette]);
            $resultats = $requete->fetchAll(PDO::FETCH_COLUMN);
    
            return $resultats;
        } catch (PDOException $e) {
            echo "Erreur lors de la récupération des quantités d'ingrédients de la recette : " . $e->getMessage();
            return [];
        }
    }

    public function getIngredientsQuantiteRecette($nomRecette) {
        try {
            $requete = $this->bdd->prepare("
                SELECT i.nom, ri.quantite
                FROM recette_ingredient ri
                INNER JOIN ingredients i ON ri.id_ingredient = i.id
                WHERE ri.id_recette = (SELECT id FROM recettes WHERE nom = ?)
            ");
            $requete->execute([$nomRecette]);
            $resultats = $requete->fetchAll(PDO::FETCH_ASSOC);
